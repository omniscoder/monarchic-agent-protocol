...
if [[ ! -x "${check_script}" ]]; then
  echo "[test-self-host-artifact-report-file-path-existence] Missing executable: ${check_script}" >&2
  exit 1
fi
...
if command -v python >/dev/null 2>&1; then
  ...
  echo "[test-self-host-artifact-report-file-path-existence] python or python3 is required" >&2
  exit 1
fi
...
if ! bash "${tmp_repo}/scripts/test-self-host-artifacts.sh" >/dev/null 2>"${stderr_log}"; then
  echo "[test-self-host-artifact-report-file-path-existence] Expected baseline fixtures to pass." >&2
  cat "${stderr_log}" >&2
  exit 1
fi
...
missing_path = "scripts/__self_host_missing_file_path_check__.sh"
...
if [[ "${exit_code}" -eq 0 ]]; then
  echo "[test-self-host-artifact-report-file-path-existence] Expected missing report file path to fail." >&2
  exit 1
fi

if ! grep -q "changed_files reference missing file path(s)" "${stderr_log}"; then
  echo "[test-self-host-artifact-report-file-path-existence] Unexpected stderr output for missing report file path check:" >&2
  cat "${stderr_log}" >&2
  exit 1
fi

echo "[test-self-host-artifact-report-file-path-existence] PASS: report file-path existence checks are deterministic."
