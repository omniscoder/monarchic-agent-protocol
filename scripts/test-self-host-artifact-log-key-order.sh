...
if [[ ! -x "${check_script}" ]]; then
  echo "[test-self-host-artifact-log-key-order] Missing executable: ${check_script}" >&2
  exit 1
fi
...
if command -v python >/dev/null 2>&1; then
  ...
  echo "[test-self-host-artifact-log-key-order] python or python3 is required" >&2
  exit 1
fi
...
if ! bash "${tmp_repo}/scripts/test-self-host-artifacts.sh" >/dev/null 2>"${stderr_log}"; then
  echo "[test-self-host-artifact-log-key-order] Expected baseline fixtures to pass." >&2
  cat "${stderr_log}" >&2
  exit 1
fi
...
if [[ "${exit_code}" -eq 0 ]]; then
  echo "[test-self-host-artifact-log-key-order] Expected key-order mismatch to fail." >&2
  exit 1
fi

if ! grep -q "Latest implementation log entry must use keys in this order" "${stderr_log}"; then
  echo "[test-self-host-artifact-log-key-order] Unexpected stderr output for key-order check:" >&2
  cat "${stderr_log}" >&2
  exit 1
fi

echo "[test-self-host-artifact-log-key-order] PASS: implementation log key-order checks are deterministic."
