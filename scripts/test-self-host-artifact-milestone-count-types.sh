...
if [[ ! -x "${check_script}" ]]; then
  echo "[test-self-host-artifact-milestone-count-types] Missing executable: ${check_script}" >&2
  exit 1
fi
...
if command -v python >/dev/null 2>&1; then
  ...
  echo "[test-self-host-artifact-milestone-count-types] python or python3 is required" >&2
  exit 1
fi
...
if ! bash "${tmp_repo}/scripts/test-self-host-artifacts.sh" >/dev/null 2>"${stderr_log}"; then
  echo "[test-self-host-artifact-milestone-count-types] Expected baseline fixtures to pass." >&2
  cat "${stderr_log}" >&2
  exit 1
fi
...
if [[ "${exit_code}" -eq 0 ]]; then
  echo "[test-self-host-artifact-milestone-count-types] Expected non-integer report total_milestones to fail." >&2
  exit 1
fi

if ! grep -q "SELF_HOST_REPORT.json total_milestones must be an integer" "${stderr_log}"; then
  echo "[test-self-host-artifact-milestone-count-types] Unexpected stderr output for report total_milestones type check:" >&2
  cat "${stderr_log}" >&2
  exit 1
fi
...
if [[ "${exit_code}" -eq 0 ]]; then
  echo "[test-self-host-artifact-milestone-count-types] Expected non-integer update milestones_done to fail." >&2
  exit 1
fi

if ! grep -q "SELF_HOST_UPDATE.json milestones_done must be an integer" "${stderr_log}"; then
  echo "[test-self-host-artifact-milestone-count-types] Unexpected stderr output for update milestones_done type check:" >&2
  cat "${stderr_log}" >&2
  exit 1
fi

echo "[test-self-host-artifact-milestone-count-types] PASS: milestone count type checks are deterministic."
