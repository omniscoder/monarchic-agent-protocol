...
if [[ ! -x "${check_script}" ]]; then
  echo "[test-self-host-artifact-latest-milestone] Missing executable: ${check_script}" >&2
  exit 1
fi
...
if command -v python >/dev/null 2>&1; then
  ...
  echo "[test-self-host-artifact-latest-milestone] python or python3 is required" >&2
  exit 1
fi
...
if ! bash "${tmp_repo}/scripts/test-self-host-artifacts.sh" >/dev/null 2>"${stderr_log}"; then
  echo "[test-self-host-artifact-latest-milestone] Expected baseline fixtures to pass." >&2
  cat "${stderr_log}" >&2
  exit 1
fi
...
if [[ "${exit_code}" -eq 0 ]]; then
  echo "[test-self-host-artifact-latest-milestone] Expected stale milestone_completed value to fail." >&2
  exit 1
fi

if ! grep -q "must reference the latest done milestone" "${stderr_log}"; then
  echo "[test-self-host-artifact-latest-milestone] Unexpected stderr output for latest milestone check:" >&2
  cat "${stderr_log}" >&2
  exit 1
fi

echo "[test-self-host-artifact-latest-milestone] PASS: latest done milestone checks are deterministic."
