...
if [[ ! -x "${check_script}" ]]; then
  echo "[test-self-host-artifact-report-new-file-path-existence] Missing executable: ${check_script}" >&2
  exit 1
fi
...
if command -v python >/dev/null 2>&1; then
  ...
  echo "[test-self-host-artifact-report-new-file-path-existence] python or python3 is required" >&2
  exit 1
fi
...
if ! bash "${tmp_repo}/scripts/test-self-host-artifacts.sh" >/dev/null 2>"${stderr_log}"; then
  echo "[test-self-host-artifact-report-new-file-path-existence] Expected baseline fixtures to pass." >&2
  cat "${stderr_log}" >&2
  exit 1
fi
...
if [[ "${exit_code}" -eq 0 ]]; then
  echo "[test-self-host-artifact-report-new-file-path-existence] Expected missing report new_files path to fail." >&2
  exit 1
fi

if ! grep -q "new_files reference missing file path(s)" "${stderr_log}"; then
  echo "[test-self-host-artifact-report-new-file-path-existence] Unexpected stderr output for missing report new_files path check:" >&2
  cat "${stderr_log}" >&2
  exit 1
fi

echo "[test-self-host-artifact-report-new-file-path-existence] PASS: report new_files path-existence checks are deterministic."
