...
if [[ ! -x "${check_script}" ]]; then
  echo "[test-self-host-artifact-log-summary-traceability] Missing executable: ${check_script}" >&2
  exit 1
fi
...
if command -v python >/dev/null 2>&1; then
  ...
  echo "[test-self-host-artifact-log-summary-traceability] python or python3 is required" >&2
  exit 1
fi
...
if ! bash "${tmp_repo}/scripts/test-self-host-artifacts.sh" >/dev/null 2>"${stderr_log}"; then
  echo "[test-self-host-artifact-log-summary-traceability] Expected baseline fixtures to pass." >&2
  cat "${stderr_log}" >&2
  exit 1
fi
...
if [[ "${exit_code}" -eq 0 ]]; then
  echo "[test-self-host-artifact-log-summary-traceability] Expected missing milestone id in summary to fail." >&2
  exit 1
fi

if ! grep -q "Latest implementation log summary must include milestone_completed id" "${stderr_log}"; then
  echo "[test-self-host-artifact-log-summary-traceability] Unexpected stderr output for summary traceability check:" >&2
  cat "${stderr_log}" >&2
  exit 1
fi

echo "[test-self-host-artifact-log-summary-traceability] PASS: summary traceability check is deterministic."
